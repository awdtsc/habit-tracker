...
/**
 * 方針（重要）:
 * - /api 側では「web系ミドルウェア（EncryptCookies/StartSession など）」を直積みしない。
 *   Kernel の 'api' グループ + Sanctum（EnsureFrontendRequestsAreStateful）に任せる。
 * - 認証必須は 'auth.api'（自作ミドルウェア）だけを掛ける。
 */
...
// SPA が最初に叩く「いまログインしてる？」（未ログインでも 200）
...
if (app()->environment('local')) {
    Route::get('/_debug/session', function (Request $r) {
        return response()->json([
            ...
            'cookies'            => $r->cookies->all(), // StartSessionは積まない（/apiでは直積みしない）
        ]);
    });

    Route::get('/_debug/auth', function (Request $request) {
        return response()->json([
            'request_user' => $request->user(),        // Sanctumで解決されたユーザー
            'cookies'      => $request->cookies->all()
        ]);
    });

    // ルート実態の可視化（/login へ飛ぶ原因の特定用）
