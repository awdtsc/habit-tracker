...
import * as api from './api'   // コンソール操作用の薄いAPIラッパ（任意）
  ...
  el.innerHTML = `
    <div class="min-h-screen flex items-center justify-center bg-gray-100 p-6">
      <div class="max-w-md w-full rounded-lg bg-white shadow p-6 text-center">
        ...
        <p class="text-sm text-gray-600 mb-4">Laravel(8000) / Vite(5173) を起動して再読み込みしてください。</p>
...
// ---------- 401イベント（bootstrapのaxiosインターセプタが発火） ----------
...
;(async () => {
  ...
  // 2) CSRF Cookie（1回だけ）※withCredentialsつき
  try {
    await axios.get('/sanctum/csrf-cookie', { withCredentials: true })
